...
function App() {
  ...
  const speakerVolumesRef =
    useRef<Record<string, number>>({})

  const mutedSpeakersRef =
  ...
  function getSpeakerName(identity: string) {
    ...
    const withoutPrefix =
      identity.slice('speaker-'.length)

    const encodedName =
      withoutPrefix.slice(0, -37)
  ...
  function applySpeakerAudio(
    ...
    audioElements.current.forEach(
      (element, identity) => {
        /*
         * AUTO:
         * Keep every connected speaker's audio available.
         * Do NOT gate the audio using active-speaker detection.
         *
         * FOCUS:
         * Hear only the selected speaker.
         */
        const shouldHear =
        ...
        const isMuted =
          muted[identity] ?? false

        const volume =
          volumes[identity] ?? 100
  ...
  function attachAudioTrack(
    ...
    const existing =
      audioElements.current.get(identity)
  ...
  function handleActiveSpeakers(
    ...
    /*
     * Active-speaker detection is now
     * VISUAL INFORMATION ONLY.
     *
     * It must never interrupt or gate
  ...
  function autoMode() {
    ...
    /*
     * Immediately restore all
     * connected speaker audio.
     */
    applySpeakerAudio(
      null,
      speakerVolumesRef.current,
  ...
  async function endConversation() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          'End conversation request failed'
        )
      }
